...
FOR INSERT OR UPDATE OR DELETE ON VOTES
COMPOUND TRIGGER
  TYPE t_film_ids IS TABLE OF NUMBER;
  v_film_ids t_film_ids := t_film_ids();
  
  AFTER EACH ROW IS
  BEGIN
    v_film_ids.EXTEND;
    v_film_ids(v_film_ids.LAST) := CASE WHEN DELETING THEN :OLD.film_id ELSE :NEW.film_id END;
  END AFTER EACH ROW;

  AFTER STATEMENT IS
  BEGIN
    FOR i IN 1 .. v_film_ids.COUNT LOOP
      UPDATE FILMS SET rating=(SELECT NVL(ROUND(AVG(score),1),0) FROM VOTES WHERE film_id=v_film_ids(i))
      WHERE id=v_film_ids(i);
    END LOOP;
  END AFTER STATEMENT;
...
CREATE OR REPLACE PROCEDURE proc_get_client_profile(
  ...
  OPEN p_cursor FOR
    SELECT f.id AS film_id, f.title, f.category,
      ...
      (SELECT category FROM (SELECT fi2.category FROM VIEWINGS vi2 JOIN FILMS fi2 ON vi2.film_id=fi2.id
       WHERE vi2.client_id=p_client_id GROUP BY fi2.category
       ORDER BY COUNT(*) DESC) WHERE ROWNUM <= 1) AS top_category,
      (SELECT NVL(ROUND(AVG(score),1),0) FROM VOTES WHERE client_id=p_client_id) AS avg_score,
      (SELECT stage_name FROM (SELECT a.stage_name FROM FILM_ACTORS fa JOIN ACTORS a ON fa.actor_id=a.id
       JOIN VIEWINGS vi3 ON vi3.film_id=fa.film_id WHERE vi3.client_id=p_client_id
       GROUP BY a.stage_name ORDER BY COUNT(*) DESC) WHERE ROWNUM <= 1) AS top_actor,
      (SELECT tag_name FROM (SELECT vt.tag_name FROM CLIENT_TAG_SELECTIONS cts
       ...
       ORDER BY COUNT(*) DESC) WHERE ROWNUM <= 1) AS top_tag
...
CREATE OR REPLACE PROCEDURE proc_recommend_films(
  ...
  OPEN p_cursor FOR
    ...
    ORDER BY f.rating DESC, total_views DESC
    ) WHERE ROWNUM <= 10;
...
CREATE OR REPLACE PROCEDURE proc_seasonal_predictions(
  ...
  OPEN p_cursor FOR
    ...
    ORDER BY view_count DESC, f.rating DESC
    ) WHERE ROWNUM <= 10;
